...
class TF2:
    def __init__(self, apiKey=None, autoUpdate=False, updateTime=24 * 60 * 60, lite=False, useBackup=True, backupMaxAge=None, fetchLockTimeout=30):
        self.apiKey = apiKey
        ...
        self.backupMaxAge = backupMaxAge
        self.fetchLockTimeout = fetchLockTimeout
    ...
    def getSchema(self):
        if self.apiKey is not None:
            maxAttempts = 3
            lastError = None
            backupMaxAge = self.backupMaxAge if self.backupMaxAge is not None else self.updateTime

            if self.useBackup:
                backupSchema = Schema.loadBackup(maxAge=backupMaxAge)
                if backupSchema is not None:
                    logger.info("Using cached backup schema")
                    self.schema = Schema(backupSchema)
                    return

            lockAcquired = Schema.acquireFetchLock(timeout=self.fetchLockTimeout)
            if not lockAcquired:
                logger.warning("Schema fetch lock not acquired; proceeding without lock")
            else:
                if self.useBackup:
                    backupSchema = Schema.loadBackup(maxAge=backupMaxAge)
                    if backupSchema is not None:
                        logger.info("Using cached backup schema after lock wait")
                        self.schema = Schema(backupSchema)
                        Schema.releaseFetchLock()
                        return
